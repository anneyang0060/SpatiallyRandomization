setwd('~/spatio blessing/single_para')
      ...
      res_c <- foreach(sd=sds[1:Nsim], .packages = c('MASS' ,'mvnfast', 'Rcpp')) %dopar% {
        ...
        for(stren in strengths){
          ...
          for(R in Rs){
            ...
            # library(MASS)
            # library(mvnfast)
            # mode=1; rho=0.9; side=3; sd=sds[1]; stren=0.005; R=36
            ...
            ### generate regions
            {
              ...
              coor <- mylist$coor
              lat <- (coor[,1]-min(coor[,1]))/(max(coor[,1])-min(coor[,1]))
              lon <- (coor[,2]-min(coor[,2]))/(max(coor[,2])-min(coor[,2]))# for determine neighbors only
            ...
            {
              set.seed(123)
              alpha <- 8+2*coeff_fn(3,lat,lon); beta <- coeff_fn(3,lat,lon) # beta <- coeff_fn(3,lat,lon)
            ...
            {
              set.seed(sd)
              e <- noise_gene(mode, sd, R, M, rho, sigma, dis_mat+diag(1e-06,R))
              X <- matrix(rnorm(R*N,EX,DX),R,N)
            }
            
            {
              ## city-level 
              res_temp <- list(model_aggre(R, design = 0,spill = 0),
                               model_aggre(R, design = 0,spill = 1))
              names(res_temp) <- c('nospill','spill')
              res0 <- c(res0, list(res_temp))
              
              ## individual randomization
              res_temp <- list(model_aggre(R, design = 1,spill = 0),
                               model_aggre(R, design = 1,spill = 1))
              names(res_temp) <- c('nospill','spill')
              res1 <- c(res1, list(res_temp))
              
              ## cluster randomization
              res_temp <- list(model_aggre(R, design = 2,spill = 0),
                               model_aggre(R, design = 2,spill = 1))
              names(res_temp) <- c('nospill','spill')
