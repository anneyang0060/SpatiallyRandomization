...
outcome_gene <- function(spill, A){
  ...
  for(i in 1:R){
    ...
    if(length(nb[i][[1]])>1){
      A_bar[i,] <- colMeans(A[nb[i][[1]],])
      X_bar[i,] <- colMeans(X[nb[i][[1]],])
    }else{
      A_bar[i,] <- A[nb[i][[1]],]
      X_bar[i,] <- X[nb[i][[1]],]
    }
    
    y[i, ] <- 5+3*(X[i,]+X_bar[i,])*sin(pi/8*(lat[i]+lon[i])+stren*A[i,]+0.5*stren*spill*A_bar[i,]) + e[i, ]
